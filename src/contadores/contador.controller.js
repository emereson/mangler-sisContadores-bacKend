...
import { Contador } from './Contador.model.js';
...
export const create = catchAsync(async (req, res, next) => {
  const { pedido } = req;
  const { array_codes, array_clacks } = req.body;

  // Procesar array_codes
  await Promise.all(
  ...
  const clackPromises = array_clacks.map(async (number_code) => {
    return await Clac.create({
      pedido_id: pedido.id,
      codigo: number_code,
    });
  });

  await Promise.all(clackPromises);
